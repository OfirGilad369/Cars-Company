    ...
    public partial class Payments : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (ans == true)
            {
                ...
                try
                {
                    ...
                    y0 = DL0.getDataTable("select * from CashPayment where Num ='" + textBox2.Text + "'", y0);
                    bool ans1 = true;
                    int i = 0;
                    while (ans1 == true)
                    {
                        if (!y0.Rows[i].Equals(null))
                        {
                            if (y0.Rows[0][3].ToString() != "מקדמה")
                            {
                                level++;
                                i++;
                            }
                ...
                try
                {
                    DAL DL0 = new DAL("CarCompany.accdb");
                    DataTable y0 = new DataTable();
                    y0 = DL0.getDataTable("select * from CheckPayment where Num ='" + textBox2.Text + "'", y0);
                    bool ans2 = true;
                    int i = 0;
                    while (ans2 == true)
                    {
                ...
                try
                {
                    DAL DL0 = new DAL("CarCompany.accdb");
                    DataTable y0 = new DataTable();
                    y0 = DL0.getDataTable("select * from CreditPayment where Num ='" + textBox2.Text + "'", y0);
                    bool ans3 = true;
                    int i = 0;
                    while (ans3 == true)
                    {
                ...
                textBox12.Text = level.ToString();

                DAL DL4 = new DAL("CarCompany.accdb");

                DataTable y4 = new DataTable();

                y4 = DL4.getDataTable("select * from OrderInfo where Num ='" + textBox2.Text + "'", y4);

                int sum = int.Parse(y4.Rows[0][2].ToString());

                try
                {
                    DAL DL0 = new DAL("CarCompany.accdb");
                    DataTable y0 = new DataTable();
                    y0 = DL0.getDataTable("select * from AccessoriesOrders where Num ='" + textBox2.Text + "'", y0);
                    ...
                    while (ans3 == true)
                    {
                        if (!y0.Rows[i].Equals(null))
                        {
                            DAL DL1 = new DAL("CarCompany.accdb");
                            DataTable y1 = new DataTable();
                            y1 = DL1.getDataTable("select * from Accessories where AccessCode ='" + y0.Rows[0][1].ToString() + "'", y1);
                            sum += int.Parse(y1.Rows[0][2].ToString());
                ...
                try
                {
                    ...
                    while (ans1 == true)
                    {
                        if (!y0.Rows[i].Equals(null))
                        {
                            paid += int.Parse(y0.Rows[0][2].ToString());
                        }
                        else
                        {
                            ans1 = false;
                ...
                try
                {
                    ...
                    while (ans2 == true)
                    {
                        if (!y0.Rows[i].Equals(null))
                        {
                            paid += int.Parse(y0.Rows[0][5].ToString());
                        }
                        else
                        {
                            ans2 = false;
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (ans == true)
                {
                    int pay = int.Parse(textBox12.Text) + 1;
                    string kind = "תשלום " + pay.ToString(); 
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            string sql1x = "UPDATE OrderInfo SET Num='" + textBox2.Text + "', Info='" + "הספקה" + "' WHERE Num= '" + textBox2.Text + "'";
            DL1x.Insert(sql1x);
            Supply S = new Supply();
            S.GetNum(textBox2.Text);
            S.ShowDialog();
        }
